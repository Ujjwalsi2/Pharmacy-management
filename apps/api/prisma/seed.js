...
const DRUG_TYPES = ['TABLET', 'CAPSULE', 'SYRUP', 'INJECTION', 'OINTMENT', 'DROPS', 'INHALER', 'OTHER'];
...
async function main() {
    ...
    for (let i = 0; i < 25; i++) {
        ...
        if (companyDrugs.length === 0)
            continue;
        const itemCount = randomInt(1, 4);
        const chosenDrugs = new Set();
        const items = [];
        let total = 0;
        for (let j = 0; j < itemCount; j++) {
            const drug = pick(companyDrugs);
            if (chosenDrugs.has(drug.id))
                continue;
